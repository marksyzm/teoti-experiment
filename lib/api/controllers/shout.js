...
var controller = {
    ...
    get: function (req, reply) {
        Shout.findOne({ _id: req.params.shoutId }, "", "user", function (err, shout) {
            if (err) { return reply(Boom.wrap(err, 400)); }
    ...
    post: function (req, reply) {
        req.payload.user = req.documents.user._id;
        req.payload.forum = 0;
    ...
    put: function (req, reply) {
        Shout.update({ _id: req.params.shoutId }, req.payload, function (err, shout) {
            if (err) { return reply(Boom.wrap(err, 400)); }
            reply(shout);
        });
    },
    del: function (req, reply) {
        Shout.del({ _id: req.params.shoutId }, function (err, shout) {
            if (err) { return reply(Boom.wrap(err, 400)); }
            reply(shout);
        });
    }
