...
function getData (err, forums) {
    ...
    async.whilst(
        ...
        function (callback) {
            ...
            var parentList = forum.parentlist ? _.map(forum.parentlist.split(","), Number) : [];
            parentList.splice(parentList.indexOf(-1), 1);
            parentList.splice(parentList.indexOf(Number(forum.forumid)), 1);

            var children = forum.childlist ? _.map(forum.childlist.split(","), Number) : [];
            children.splice(children.indexOf(-1), 1);
            children.splice(children.indexOf(Number(forum.forumid)), 1);
