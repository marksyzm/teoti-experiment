...
class Post extends Model {
  ...
  find(query, sort, populate, cb) {
    ...
    const find = _.omit(query, ['page','sort','limit','user','firstPostId']);
  ...
  save(data, query, cb) {
    if (!security.hasPermission(data.user, "create", "own", "post")) {
      return cb(new Error("Invalid post create permission"));
    }

    async.waterfall([
      this.validateThread.bind(this, data, query),
  ...
  validateThread(post, query, callback) {
    Thread.validate(post.thread, post.forum._id, (err) => callback(err, post, query));
  }
  ...
  updateForum(data, post, callback) {
    Forum.update({_id: data.documents.forum._id }, {lastPost: post._id}, (err) => callback(err, post));
  }
  ...
  update(query, data, cb) {
    async.waterfall([
      this.validateThread.bind(this, data, query),
      // get original post content, check user in data is same as session user
      // update
      super.update.bind(this, data),
