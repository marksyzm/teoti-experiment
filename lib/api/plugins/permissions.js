...
function canPass(permissionSettings, crudType, user) {
    ...
    ["all", "own"].every(function (accessRange) {
        console.log(!_.isEmpty(permission[accessRange]), !_.isEmpty(user.group[crudType][accessRange]));
        if (!_.isEmpty(permission[accessRange]) && !_.isEmpty(user.group[crudType][accessRange])) {
            ...
            console.log(accessTypes, permission[accessRange], user.group[crudType][accessRange]);
