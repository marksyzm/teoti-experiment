...
var topLevelForumIds = [15, 1, 2, 3];

var mysqlClient;
var callback;
var size = 500;

function getGroups (forumId) {
    switch (forumId) {
        case 1: return [ 1 ];
        case 1: return [ 1 ];
        default: return [];
...
function getData (err, forums) {
    ...
    async.whilst(
        ...
        function (callback) {
            ...
            children.splice(children.indexOf(forum.forumid), 1);
            ...
            var data = {
                ...
                groups          : getGroups(forum.forumid),
                public          : publicForumIds.indexOf(forum.forumid) !== -1,
                override        : topLevelForumIds.indexOf(forum.forumid) !== -1
            };
            ...
            Forum.create(data, function (err, forum) {
                ...
                console.info( "Created: id: ", forum._id, "Title:", forum.title );
                callback(null);
            });
        },
        function (err) {
